...
def combined_bio_upload():
    ...
    try:
         row, credits = get_user_row(final_uid)
    except Exception as e:
         return jsonify({
             "status": "Credit Server Error",
             "message": str(e)
         }), 500
    ...
    if credits <= 0:
         return jsonify({
             "status": "No Credits",
             "message": "Your credits are finished. Contact owner."
         }), 403
    result = upload_bio_request(final_jwt, bio)
    if result["code"] == 200:
       sheet.update_cell(row, 2, credits - 1)
    
    response_data = {
        "Credit": "Nivashini",
        "remaining_credits": credits if result["code"] != 200 else credits - 1,
        "Insta": "ft_rosie._",
