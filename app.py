...
jwt_collection = db["saved_jwts"]
jwt_collection.create_index("uid", unique=True)
try:
...
def save_jwt(uid, name, access_up):
...
def get_correct_name(uid):
    try:
        url = f"https://ffinfo-livid.vercel.app/accinfo?uid={uid}"
...
def combined_bio_upload():
    ...
    if Liked == "Success":
         save_jwt(final_uid, final_name, final_access_token)
