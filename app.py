...
def get_user_credits(uid):
    ...
    user = credits_collection.find_one(
        {
            "uid": str(uid)
        }
    )
...
def send_single_like_request(token_value):
    uid = 122809569
    ooru = "IND"

    like_protobuf_payload = create_protobuf_message(uid, ooru)

    encrypted_like_payload = encrypt_message(AES_KEY, AES_IV, like_protobuf_payload)

    # encrypt_message already returns BYTES → no fromhex()
    edata = encrypted_like_payload
    ...
    try:
        response = requests.get(url, data=edata, headers=headers, timeout=10)
        ...
        print("Like request timed out")
        return 998

    except Exception as e:
        print(f"Exception in send_single_like_request: {e}")
        return 997
...
def bind_info_endpoint():
    ...
    if access:
        ...
        access_token = get_garena_data(eat)
    elif not access or eat:
        return jsonify({
            "status": "error",
            "error": "access or eat tokens required"
        }), 400
...
def combined_bio_upload():
    ...
    if jwt_token:
        ...
        if eat.startswith("https"):
          eat_token = eat.split("eat=")[1].split("&")[0]
        else:
          eat_token = eat
        login_method = "Eat Token Login"
        final_access_token = get_garena_data(eat_token)
...
def combined_token_upload():
    ...
    if uid and password:
        ...
        return jsonify({"status": "❌ Error", "code": 400, "error": "Provide UID/Pass, or Eat Token, or Access Token"}), 400
...
def add_credit_api():
    ...
    if not uid or not amount:
        return jsonify({
            "status": "Missing uid or amount"
        }), 400

    add_credit(uid, int(amount))
...
.content {
    flex: 1;
    padding: 24px 16px calc(300px + env(safe-area-inset-bottom));
}
...
.sticky-insta a:active {
    background-color: rgba(255,255,255,0.18);
    box-shadow:
        inset 0 2px 6px rgba(255,255,255,0.45),
        0 8px 22px rgba(246,193,209,0.35);
}
...
const uid =
document.getElementById("credit_uid").value;

const result =
document.getElementById("credit_result");

if(!uid){
result.innerHTML="⚠️ Enter UID";
return;
}
...
const res =
await fetch(`/checkcredit?uid=${uid}`);
