...
.content {
    flex: 1;
    padding: 24px 16px 240px; /* SAFE space for Insta + Help */
}
...
.help-fab {
    position: fixed;
    bottom: 170px; /* safely above Instagram bar */
    right: 16px;
    z-index: 9996;
    text-align: right;
    pointer-events: none;
...
function toggleHomeHelp() {
    document.getElementById("homeEatHelp").classList.toggle("show");
}

function closeHomeHelp(e) {
    e.stopPropagation();
    document.getElementById("homeEatHelp").classList.remove("show");
}

function openEatVideo(e) {
    e.stopPropagation();
    window.location.href = "https://youtube.com/shorts/7MzmoPxpcVU?si=YAFipIFm2tbgEOrn";
}

/* Hide help when keyboard opens (mobile-safe) */
window.addEventListener("focusin", () => {
    document.querySelector(".help-fab").style.display = "none";
});
window.addEventListener("focusout", () => {
    document.querySelector(".help-fab").style.display = "block";
});
...
<!-- 🌸 Floating Help -->
<div class="help-fab">
    <div class="help-toggle" onclick="toggleHomeHelp()">
        🎥 Watch Tutorial
    </div>
    <div class="help-actions" id="homeEatHelp">
        <button class="help-btn play" onclick="openEatVideo(event)">▶️ Play</button>
        <button class="help-btn close" onclick="closeHomeHelp(event)">✖ Close</button>
    </div>
</div>
